...
setGlobalOptions({ region: process.env.FUNCTION_REGION || "us-central1", timeoutSeconds: 120, memory: "1GiB" });

const auth = getAuth();
const httpOptions = { cors: true } as const;
...
function dedupeKeyForTransaction(input: { date: string; merchant: string; amount: number }) {
...
async function requireUser(req: any): Promise<AuthenticatedUser> {
...
function respondJson(res: any, status: number, payload: JsonMap | JsonMap[]) {
  res.status(status).set("Content-Type", "application/json").send(JSON.stringify(payload));
}

function respondError(res: any, status: number, message: string, details?: unknown) {
...
function defaultProfile(uid: string, email: string | null): JsonMap {
...
function normalizeProfile(uid: string, email: string | null, incoming: JsonMap, existing?: JsonMap | null) {
  ...
  return {
    ...
    password_setup_completed: asBoolean(
      incoming.password_setup_completed ?? base.password_setup_completed,
      true,
    ),
    ...
    monthly_fixed_expenses: asNumber(
      incoming.monthly_fixed_expenses ?? base.monthly_fixed_expenses,
    ),
    ...
    subscription_awareness: asString(
      incoming.subscription_awareness ?? base.subscription_awareness,
    ),
    target_monthly_savings: asNumber(
      incoming.target_monthly_savings ?? base.target_monthly_savings,
    ),
    onboarding_completed: asBoolean(
      incoming.onboarding_completed ?? base.onboarding_completed,
      false,
    ),
...
async function getProfile(uid: string, email: string | null): Promise<JsonMap | null> {
...
async function saveProfile(uid: string, email: string | null, incoming: JsonMap, merge = true) {
  const existing = await getProfile(uid, email);
...
async function listDocuments(uid: string, collectionName: string) {
  const snapshot = await userCollection(uid, collectionName).get();
  return snapshot.docs.map((doc) => ({ id: doc.id, ...sanitize(doc.data()) }));
}

async function replaceCollection(uid: string, collectionName: string, items: JsonMap[]) {
  ...
  for (const item of items) {
    const docId = asString(item.id) || randomUUID();
...
async function saveCollectionDoc(uid: string, collectionName: string, payload: JsonMap) {
  const docId = asString(payload.id) || randomUUID();
  const ref = userCollection(uid, collectionName).doc(docId);
...
async function deleteCollectionDoc(uid: string, collectionName: string, docId: string) {
  if (!docId) return;
  await userCollection(uid, collectionName).doc(docId).delete();
}

function monthlySubscriptionCost(subscriptions: JsonMap[]) {
...
function latestSpendingDate(spendingEvents: JsonMap[]) {
  if (!spendingEvents.length) return null;
  return spendingEvents
    .map((event) => toIsoDate(event.date))
    .sort((left, right) => right.localeCompare(left))[0] ?? null;
}

function buildDashboardSummary(profile: JsonMap, financialEntries: JsonMap[], subscriptions: JsonMap[], spendingEvents: JsonMap[]) {
...
function aggregateCategorySpend(spendingEvents: JsonMap[], startDate: Date) {
...
function buildBudgetStatuses(budgetLimits: JsonMap[], spendingEvents: JsonMap[]) {
  ...
  return budgetLimits.map((limit) => {
    ...
    let status: "healthy" | "watch" | "over" = "healthy";
    if (percentUsed >= 100) {
      status = "over";
...
function buildGoalStatuses(goals: JsonMap[]) {
  const now = new Date();
  return goals.map((goal) => {
    ...
    const monthlyContributionNeeded =
      daysRemaining > 0 ? remainingAmount / Math.max(1, daysRemaining / 30) : remainingAmount;
    let status: "on_track" | "needs_attention" | "achieved" = "on_track";
...
function buildPatternSummaries(spendingEvents: JsonMap[]) {
...
function buildForecast(profile: JsonMap, dashboardSummary: JsonMap, spendingEvents: JsonMap[]) {
  ...
  let status: "needs_more_data" | "on_track" | "watch" | "overextended" = "on_track";
...
function buildSubscriptionReview(subscriptions: JsonMap[], profile: JsonMap) {
...
function buildAdvice(bootstrap: JsonMap) {
  const advice: JsonMap[] = [];
  ...
  const budgetWarning = (bootstrap.budget_statuses || []).find((status: JsonMap) => status.status !== "healthy");
  if (budgetWarning) {
    advice.push({
  ...
  const goalAttention = (bootstrap.goal_statuses || []).find((goal: JsonMap) => goal.status === "needs_attention");
  if (goalAttention) {
    advice.push({
...
function buildSummaryResult(period: SummaryPeriod, spendingEvents: JsonMap[]) {
  const now = new Date();
...
function buildAffordabilityResult(input: JsonMap, bootstrap: JsonMap) {
  ...
  let status: "comfortable" | "tight" | "not_now" | "needs_more_data" = "comfortable";
...
function buildEmptyFlags(bootstrap: JsonMap) {
  return {
...
async function buildBootstrap(uid: string, email: string | null) {
...
async function replaceOnboardingData(uid: string, email: string | null, payload: JsonMap) {
  ...
  await Promise.all([
    ...
    replaceCollection(
      uid,
      COLLECTIONS.budgetLimits,
      Array.isArray(payload.budget_limits) ? payload.budget_limits : [],
    ),
    replaceCollection(
      uid,
      COLLECTIONS.financialEntries,
      Array.isArray(payload.financial_entries) ? payload.financial_entries : [],
    ),
    replaceCollection(
      uid,
...
function guessCategory(description: string) {
  ...
  if (/uber|taxi|matatu|transport|fuel|gas/.test(text)) return "Transport";
  if (/rent|mortgage|airbnb|apartment/.test(text)) return "Housing";
  if (/food|lunch|dinner|breakfast|restaurant|grocer|market|coffee/.test(text)) return "Food";
  if (/netflix|spotify|subscription|icloud|youtube/.test(text)) return "Subscriptions";
  if (/hospital|clinic|medicine|doctor|pharmacy/.test(text)) return "Health";
  if (/school|course|book|tuition/.test(text)) return "Education";
  if (/cloth|shoe|fashion/.test(text)) return "Shopping";
  if (/electric|water|utility|internet|wifi/.test(text)) return "Utilities";
  return "Other";
}
...
async function collectExistingDedupeKeys(uid: string) {
...
async function createNotification(uid: string, payload: JsonMap) {
...
async function importCsvTransactions(uid: string, csvText: string, fileName: string | null) {
...
async function reviewDraftTransaction(uid: string, input: JsonMap) {
...
function parseMerchantFromText(text: string) {
  const patterns = [
...
async function ingestForwardedReceipt(payload: JsonMap) {
...
function guessNewsCategory(result: JsonMap) {
  const text = `${asString(result.title)} ${asString(result.content)}`.toLowerCase();
  if (/crypto|bitcoin|ethereum|solana/.test(text)) return "Crypto";
  if (/mortgage|housing|rent|real estate/.test(text)) return "Real Estate";
  if (/inflation|fed|economy|jobs|gdp/.test(text)) return "Economy";
  if (/budget|saving|credit|debt|retirement/.test(text)) return "Personal Finance";
  if (/oil|gold|commodity/.test(text)) return "Commodities";
  if (/policy|regulation|congress|tax/.test(text)) return "Policy";
  if (/ai|tech|apple|microsoft|google|meta|nvidia/.test(text)) return "Tech";
  return "Markets";
}

function guessSentiment(text: string) {
  const normalized = text.toLowerCase();
  if (/surge|gain|beat|rally|upgrade|bull/i.test(normalized)) return "bullish";
  if (/drop|fall|miss|downgrade|bear|warning/i.test(normalized)) return "bearish";
  return "neutral";
}

async function callTavily(query: string, maxResults = 6) {
  ...
  if (!response.ok) {
    logger.warn("Tavily request failed", { status: response.status });
    return [];
  }
...
async function callGeminiJson<T>(prompt: string): Promise<T | null> {
  ...
  const payload = (await response.json()) as JsonMap;
  const text = payload.candidates?.[0]?.content?.parts?.map((part: JsonMap) => asString(part.text)).join("\n") || "";
  if (!text.trim()) {
    return null;
  }
...
async function buildStockRecommendations() {
  const results = await callTavily(
    "recent analyst upgrades stock watchlist motley fool wall street research high quality companies",
    8,
  );
  if (!results.length) {
    return {
  ...
  const generated = await callGeminiJson<JsonMap>(`
...
function buildInsightAnalysis(bootstrap: JsonMap, frequency: string) {
  ...
  const relevantEvents = spendingEvents.filter((event: JsonMap) => new Date(toIsoDate(event.date)) >= start);
  ...
  const healthyBudget = (bootstrap.budget_statuses || []).find((status: JsonMap) => status.status === "healthy");
  if (healthyBudget) {
    insights.push({
  ...
  return {
    ...
    savings_opportunity:
      bootstrap.subscription_review?.recommendations?.reduce(
        (sum: number, item: JsonMap) => sum + asNumber(item.monthly_impact),
        0,
      ) || 0,
  };
}

function buildFinancialStatement(bootstrap: JsonMap) {
  ...
  const incomeItems = entries
    .filter((entry: JsonMap) => entry.entry_type === "asset" && asNumber(entry.cashflow) > 0)
    .map((entry: JsonMap) => ({
  ...
  const assets = entries.filter((entry: JsonMap) => entry.entry_type === "asset").map((entry: JsonMap) => ({
    name: asString(entry.name),
  ...
  const liabilities = entries.filter((entry: JsonMap) => entry.entry_type === "liability").map((entry: JsonMap) => ({
    name: asString(entry.name),
    type: asString(entry.type) || "other",
...
function createStreamChunk(content: string) {
  return JSON.stringify({ choices: [{ delta: { content } }] });
}

function streamSse(res: any, payload: JsonMap) {
  res.write(`data: ${JSON.stringify(payload)}\n\n`);
}

function streamText(res: any, text: string) {
...
function parseSpendingMessage(message: string) {
  ...
  const items = segments
    ...
    .filter(Boolean) as Array<{ category: string; amount: number; description: string }>;
...
async function logChatSpending(uid: string, rawInput: string, parsed: JsonMap) {
  const eventRef = userCollection(uid, COLLECTIONS.spendingEvents).doc();
...
function helpLink(articleId: string) {
  return `${SUPPORT_BASE_URL}/${articleId}`;
}

function buildChatReply(message: string, bootstrap: JsonMap, parsedSpending: JsonMap | null, affordability: JsonMap | null) {
  if (parsedSpending) {
    const latestBudgetWarning = (bootstrap.budget_statuses || []).find((status: JsonMap) => status.status !== "healthy");
...
async function runScheduledSummaryPass(period: SummaryPeriod) {
...
export const financeCore = onRequest(httpOptions, async (req, res) => {
  try {
    const user = await requireUser(req);
    const body = (req.body && typeof req.body === "object" ? req.body : {}) as JsonMap;
...
export const generateStatement = onRequest(httpOptions, async (req, res) => {
  try {
    const user = await requireUser(req);
    const bootstrap = await buildBootstrap(user.uid, user.email);
...
export const generateInsights = onRequest(httpOptions, async (req, res) => {
  try {
    const user = await requireUser(req);
...
export const fetchFinanceNews = onRequest(httpOptions, async (_req, res) => {
  try {
...
export const stockRecommendations = onRequest(httpOptions, async (_req, res) => {
  try {
...
export const chat = onRequest(httpOptions, async (req, res) => {
  try {
    const user = await requireUser(req);
    const body = (req.body && typeof req.body === "object" ? req.body : {}) as JsonMap;
...
export const receiptIngress = onRequest(httpOptions, async (req, res) => {
  try {
    ...
    return respondJson(res, 200, await ingestForwardedReceipt((req.body || {}) as JsonMap));
  } catch (error) {
    return respondError(res, 400, error instanceof Error ? error.message : "Unable to ingest receipt.", error);
  }
});

export const scheduledSummaries = onRequest(httpOptions, async (req, res) => {
  try {
    ...
    const period = (asString((req.body || {}).period) || "daily") as SummaryPeriod;
...
export const scheduledSummariesDaily = onSchedule(
  {
    schedule: "0 7 * * *",
...
export const scheduledSummariesWeekly = onSchedule(
  {
    schedule: "0 8 * * MON",
    timeZone: "Africa/Nairobi",
  },
